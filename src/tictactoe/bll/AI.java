...
*/
        /*System.out.println("BLOCKING DIAG : ");
        for (BlockingPosition buttonName : blockingDiagPosition) {
            System.out.println(" Button name : "+buttonName.getButton()
                   ...
                   // +" free place On ROW : "+buttonName.getFreePlacesOnRow()
                    ...
                    +" Ai in Diag : "+buttonName.getNumAIInDiag()
                   // +" Human player In ROW : "+buttonName.getNumHumanPlayerInRow()
                   // +" Human player in COL : "+buttonName.getNumHumanPlayerInCol()
                   // +" AI player in ROW :"+buttonName.getNumAIPlayerInRow()
                  //  +" AI player in COL :"+ buttonName.getNumAIPlayerInCol()
        ...
        System.out.println("BLOCKING INV DIAG : ");
        for (BlockingPosition buttonName : blockingInvDiagPosition) {
            System.out.println(" Button name : "+buttonName.getButton()
                   ...
                   // +" free place On  COL : "+buttonName.getFreePlacesOnCol()
                    ...
                    +" Longest Human in Inv Diag ; "+buttonName.getUninterruptedPlayerinDiag()
                    +" AI in Inv Diag : "+buttonName.getNumAIInInvDiag()
                    //+" Human player In ROW : "+buttonName.getNumHumanPlayerInRow()
                    //+" Human player in COL : "+buttonName.getNumHumanPlayerInCol()
                    //+" AI player in ROW :"+buttonName.getNumAIPlayerInRow()
                    //+" AI player in COL :"+ buttonName.getNumAIPlayerInCol()
            ...
            for (int i = 0; i < handPlayed.length-dp.getHeight()+1; i++) {
                ...
                for (int k = 0; k < handPlayed.length-dp.getWidth()+1; k++) {
                    ...
                    for (int j = 0; j < dp.rowPositions.size(); j++) {
                       ...
                       /* if(handPlayed[i+dp.rowPositions.get(j)][k+dp.colPositions.get(j)] != player2) {


                            if(handPlayed[i+dp.rowPositions.get(j)][k+dp.colPositions.get(j)] == player) {
                                matchPatternPlayer++;
                                System.out.println(" Design : " + dp.getName() + " match with points at row : " + (i + dp.rowPositions.get(j)) + " col : " + (k + dp.colPositions.get(j)) + " J = " + j);
                            } else {
                                if(dp.getName().equals("InvDiagonal")) {
                                    System.out.println(" Design : " + dp.getName() + " match with points at row : " + (i + dp.rowPositions.get(j)) + " col : " + (k + dp.colPositions.get(j)) + " J = " + j);
                                    matchPattern = matchPatternPlayer;
                                    matchPatternPlayer = 0;
                                }
                            }
                            if(handPlayed[i+dp.rowPositions.get(j)][k+dp.colPositions.get(j)] == null && matchPattern>=2){
                                if(checkRow==0 && checkCol==0) {

                                    checkRow = i + dp.rowPositions.get(j);
                                    checkCol = k + dp.colPositions.get(j);
                                    emergencyOn=true;
                                    System.out.println("In initializing checks for design : "+dp.getName()+" with coordinate : row : "+checkRow+" col : "+checkCol);
                                }
                            }
                        }*/
       /*                 if(handPlayed[i+dp.rowPositions.get(j)][k+dp.colPositions.get(j)] == player) {
                    ...
                    /*if((matchPattern==2 || matchPattern==3) && emergencyOn) {
                       // System.out.println("Matching");
                        System.out.println("Match pattern = "+matchPattern+" Pattern = "+dp.getName());
                        System.out.println("In emergency row and col, row=> "+checkRow+" Col => "+checkCol+" Pattern "+dp.getName());
                        emergencyRow=checkRow;
                        emergencyCol=checkCol;


                        if(handPlayed[emergencyRow][emergencyCol]==null) {
                            System.out.println("In emergency creation row and col, row=> "+emergencyRow+" Col => "+emergencyCol+" Pattern "+dp.getName());
                            emergencyPlay= new BlockingPosition(getButtonText("L_" + emergencyRow + "_C_" +
                                            emergencyCol),emergencyRow,emergencyCol);
                            emergencyRow=0;
                            emergencyCol=0;
                        }
                    }
